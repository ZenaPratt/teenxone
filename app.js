...
var teen_xoneDb = require('./assets/js/mongo').teen_xoneDb;
...
// start DB
var teen_xoneDb = new teen_xoneDb('localhost', 27017);
teen_xoneDb.addProviders();
...
app.get('/service/:id', function(req, res) {
  ...
  teen_xoneDb.getServiceList( function(error,services){
        contents.sidebar = services;
        teen_xoneDb.getService(req.params.id, function(error,service){
...
app.get('/service', function(req, res) {
  ...
  teen_xoneDb.getServiceList( function(error,services){
        contents.sidebar = services;
        teen_xoneDb.getService(services[0]._id, function(error,service){
          contents.returnItem = service;
          res.render('services', contents);
        });
  });
...
app.post('/service', function(req, res) {
  ...
  teen_xoneDb.addService(service, function(error, record) {
    if(error){
      res.send(500);
    }else{
...
app.post('/service/:id', function(req, res) {
  ...
  service.type = 'service';
  teen_xoneDb.updateService(req.param('id'), service, function(error) {
    if(error){
      res.send(500);
    }else{
      res.redirect('/admin/services');
...
app.delete('/service/:id', function (req, res) {
  teen_xoneDb.deleteService(req.param('id'), function(error, object){
...
app.get('/product/:id', function(req, res) {
  ...
  teen_xoneDb.getProductList( function(error,products){
        contents.sidebar = products;
        contents.sideName = "Products";
        contents.sideDescription = "Products available in the store.";
        teen_xoneDb.getProduct(req.params.id, function(error,product){
...
app.get('/product', function(req, res) {
  var contents = { products:"active",
	...
	teen_xoneDb.getProductList( function(error,products){
        ...
        contents.sideName = "Products";
        contents.sideDescription = "Products available in the store.";
        teen_xoneDb.getProduct(products[0]._id, function(error,product){
          contents.returnItem = product;
          res.render('products', contents);
        });
	});
...
app.post('/product/:id', function(req, res) {
  ...
  teen_xoneDb.updateProduct(req.param('id'), product, function(error) {
    if(error){
      res.send(500);
    }else{
      res.redirect('/admin');
...
app.post('/product', function(req, res) {
  ...
  product.type = 'product';
  teen_xoneDb.addProduct(product, function(error, record) {
    if(error){
      res.send(500);
    }else{
      console.log("Added: "+record.name+" : "+record._id );
...
app.delete('/product/:id', function (req, res) {
  teen_xoneDb.deleteProduct(req.param('id'), function(error, object){
    res.send({ 'id': req.param('id')});
  });
});
